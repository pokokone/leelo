...
// Server metadata
export const SERVER_NAME = 'leelo';
...
export async function createMCPServer() {
  ...
  console.error(`🛡️  Auth: Installed-App (Client ID Only)`);
  console.error(`⏱️  Rate limit: 60 requests/minute`);
...
Rate limits: 60 requests/minute. Cache TTL: ${cacheTTL / 60000} minutes.`,
    },
    {
      capabilities: {
        tools: {},
...
export async function startHttpServer(port: number = 3000) {
  ...
  httpServer.listen(port, () => {
    ...
    console.error('💡 Tip: Higher rate limits are available by default with your Client ID\n');
  });
}